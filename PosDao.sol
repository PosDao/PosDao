...
contract MainToken is IERC20, Ownable {
    ...
    constructor() {
        ...
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x610178dA211FEF7D417bC0e6FeD39F05609AD788);
    ...
    function excludeDividendExempt(address account,bool _bool) public onlyOwner {
        isDividendExempt[account] = _bool;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool isPresaleAddress=false;
        if( _presaleAddress == from || _presaleAddress == to){
            isPresaleAddress=true;
        }
        //indicates if fee should be deducted from transfer
        bool takeFee = true;

        //if any account belongs to _isExcludedFromFee account then remove the fee
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || to == uniswapV2Pair || from == address(uniswapV2Router) || isPresaleAddress) {
            takeFee = false;
        }
        ...
        from != uniswapV2Pair && !isPresaleAddress;
    ...
    function _takeburnFee(address sender, uint256 tAmount) private {
        ...
        if (_burnTotal >= 999 * 10 ** 8 * 10 ** 9) _burnFee = 0;
    ...
    function _takeInviterFee(address sender, address recipient, uint256 tAmount) private {
        ...
        cur = inviter[cur];
        if (cur != address(0)) {
            uint256 curTAmount = tAmount.div(10000).mul(_inviterFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        if(_presaleAddress != sender && _presaleAddress != recipient && sender != owner()){
            tAmount=tAmount.sub(tAmount.div(10000));
        }
    ...
    function burn(uint256 tAmount) external {
        tAmount=tAmount.mul(10**_decimals);
